...
class BasicTextGenerator:
    ...
    def prompt(self, gender):
        task_descriptions = ', '.join([list(task.keys())[0] for task in tasks])

        user_msg = (
            f"Create a daily routine for an individual who is {gender}."
...
if __name__ == "__main__":
    ...
    base_output_dir = f'{gender}_output_raw'  # Updated directory
    ...
    for i in range(50):
        ...
        response_content = text_generator.prompt(gender)

        if response_content:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            # Updated filename to save as .txt
            filename = os.path.join(base_output_dir,
                                    f'{i + 1}_{gender}_output_{timestamp}.txt')
