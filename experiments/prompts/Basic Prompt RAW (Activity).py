...
class BasicTextGenerator:
    ...
    def prompt(self):
        activity_descriptions = ', '.join([list(task.keys())[0] for task in tasks])

        user_msg = (
            f"Create a daily routine using activities selected from the following list:\n{activity_descriptions}\n"
            "The output must be in the following JSON format with double "
            "quotes for keys: " '{"schedule": [{"time": time, "activity": activity}]}'
        )
...
if __name__ == "__main__":
    ...
    base_output_dir = '../outputs/basic_raw_set1_activity'  # Updated directory
    ...
    for i in range(500):
        ...
        if response_content:
            ...
            filename = os.path.join(base_output_dir, f'{i + 1}_output_basic_activity_{timestamp}.txt')
