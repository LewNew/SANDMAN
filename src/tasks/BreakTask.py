...
class BreakTaskMemoryBlock(MemoryDataBlock):
...
class BreakTask(Task):
    ...
    def __init__(self, config, context, lower_time=5, upper_time=10,**kwargs):
        ...
        self._name = "BreakTask-"+self._name
    ...
    def do_work(self,persona=None,mood=None,memory=None):
        ...
        if not memory == None:
            if not 'BreakTask' in memory:
                memory['BreakTask'] = MemoryList(2)

            new_ntmb = BreakTaskMemoryBlock(self._name)
            memory['BreakTask'].append(new_ntmb)
